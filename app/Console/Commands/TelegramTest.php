...
class TelegramTest extends Command
{
    ...
    /**
     * Тестировать сценарии сообщений для бота
     */
    private function testMessageScenarios()
    {
        ...
        if (!$testUser) {
            ...
            $this->info("✅ Создан тестовый пользователь: {$testUser->name}");
        }

        // Тестируем систему токенов
        $this->testTokenSystem($testUser);

        $this->info('📖 Доступные сценарии для тестирования:');
        $this->newLine();

        $scenarios = [
            '1. /start без параметров (новый пользователь)' => 
                '💡 Должен показать общее приветственное сообщение с опцией создания аккаунта',
            '2. /start без параметров (существующий пользователь)' => 
                '💡 Должен показать персонализированное приветствие',
            '3. /start с корректным токеном' => 
                '💡 Должен связать аккаунт через безопасный токен',
            '4. /start с некорректным токеном' => 
                '💡 Должен показать ошибку недействительного токена',
            '5. Текстовое сообщение от связанного пользователя' => 
                '💡 Должен показать меню с кнопками ЛК и поддержки',
            '6. Текстовое сообщение от несвязанного пользователя' => 
                '💡 Должен предложить связать аккаунт или создать новый'
        ];
    ...
    /**
     * Тестировать систему токенов
     */
    private function testTokenSystem(User $user)
    {
        $this->info('🔐 Тестирование системы токенов связки:');
        $this->newLine();

        try {
            // Создаем токен
            $token = \App\Models\TelegramLinkToken::generateForUser($user);
            $this->info("✅ Токен создан: {$token->token}");
            $this->info("   Истекает: {$token->expires_at->format('d.m.Y H:i')}");
            $this->info("   Пользователь: {$user->name}");

            // Проверяем поиск токена
            $foundToken = \App\Models\TelegramLinkToken::findValidToken($token->token);
            if ($foundToken) {
                $this->info("✅ Токен найден успешно");
            } else {
                $this->error("❌ Токен не найден");
            }

            // Проверяем ссылку для бота
            $botUsername = config('telegram.bot_username', 'testbot');
            $botLink = "https://t.me/{$botUsername}?start={$token->token}";
            $this->info("🔗 Ссылка для бота: {$botLink}");

            $this->newLine();

        } catch (\Exception $e) {
            $this->error("❌ Ошибка тестирования токенов: {$e->getMessage()}");
        }
    }
} 