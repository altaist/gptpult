...
class PaymentController extends Controller
{
    ...
    /**
     * Обработать возврат с ЮКассы после оплаты для Telegram WebApp
     *
     * @param Request $request
     * @param int $orderId
     * @return \Illuminate\Http\RedirectResponse
     */
    public function handleTelegramPaymentComplete(Request $request, int $orderId)
    {
        try {
            $order = Order::with('document', 'user')->find($orderId);

            if (!$order) {
                Log::error('Заказ не найден при Telegram возврате с ЮКассы', [
                    'order_id' => $orderId,
                    'request_params' => $request->all()
                ]);
                
                // Для Telegram направляем в WebApp
                $telegramUrl = $this->buildTelegramWebAppUrl('/lk?payment_error=order_not_found');
                return redirect($telegramUrl);
            }

            // Проверяем права доступа
            if ($order->user_id !== Auth::id()) {
                Log::warning('Попытка доступа к чужому заказу при Telegram возврате с ЮКассы', [
                    'order_id' => $orderId,
                    'order_user_id' => $order->user_id,
                    'current_user_id' => Auth::id()
                ]);
                
                $telegramUrl = $this->buildTelegramWebAppUrl('/lk?payment_error=access_denied');
                return redirect($telegramUrl);
            }

            Log::info('Telegram возврат с ЮКассы', [
                'order_id' => $orderId,
                'user_id' => Auth::id(),
                'request_params' => $request->all()
            ]);
            ...
            if (!$payment) {
                Log::warning('Платеж не найден при Telegram возврате с ЮКассы', [
                    'order_id' => $orderId
                ]);
                
                $telegramUrl = $this->buildTelegramWebAppUrl('/lk?payment_error=payment_not_found');
                return redirect($telegramUrl);
            }
            ...
            $yookassaPaymentId = $payment->payment_data['yookassa_payment_id'] ?? null;
            if (!$yookassaPaymentId) {
                Log::error('ID платежа ЮКассы не найден при Telegram возврате', [
                    'order_id' => $orderId,
                    'payment_id' => $payment->id
                ]);
                
                $telegramUrl = $this->buildTelegramWebAppUrl('/lk?payment_error=payment_id_not_found');
                return redirect($telegramUrl);
            }
            ...
            try {
                ...
                Log::info('Информация о платеже получена из ЮКассы для Telegram', [
                    'order_id' => $orderId,
                    'payment_id' => $yookassaPaymentId,
                    'status' => $paymentInfo['status'],
                    'amount' => $paymentInfo['amount']
                ...
                if ($paymentInfo['status'] === 'succeeded') {
                    ...
                    if ($order->status === OrderStatus::PAID) {
                        Log::info('Заказ уже оплачен при Telegram возврате с ЮКассы', [
                            'order_id' => $orderId,
                            'payment_id' => $paymentInfo['id']
                        ]);
                    } else {
                    ...
                    $redirectPath = $order->document_id 
                        ? '/documents/' . $order->document_id . '?payment_success=1'
                        : '/lk?payment_success=1';
                        
                    $telegramUrl = $this->buildTelegramWebAppUrl($redirectPath);
                    return redirect($telegramUrl);
                }
                ...
                if ($paymentInfo['status'] === 'waiting_for_capture') {
                    $redirectPath = $order->document_id 
                        ? '/documents/' . $order->document_id . '?payment_pending=1'
                        : '/lk?payment_pending=1';
                ...
                // Другие статусы
                $redirectPath = $order->document_id 
                    ? '/documents/' . $order->document_id . '?payment_status=' . urlencode($paymentInfo['status'])
                    : '/lk?payment_status=' . urlencode($paymentInfo['status']);
                    
                $telegramUrl = $this->buildTelegramWebAppUrl($redirectPath);
                return redirect($telegramUrl);

            } catch (Exception $e) {
                Log::error('Ошибка при получении информации о платеже из ЮКассы для Telegram', [
                    'order_id' => $orderId,
                    'payment_id' => $yookassaPaymentId,
                    'error' => $e->getMessage()
                ]);
                ...
                $redirectPath = $order->document_id 
                    ? '/documents/' . $order->document_id . '?payment_error=check_failed'
                    : '/lk?payment_error=check_failed';
            ...
            Log::error('Критическая ошибка при обработке Telegram возврата с ЮКассы', [
                'order_id' => $orderId,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);

            $telegramUrl = $this->buildTelegramWebAppUrl('/lk?payment_error=critical_error');
            return redirect($telegramUrl);
        }
    }

    /**
     * Построить URL для Telegram WebApp
     *
     * @param string $path
     * @return string
     */
    private function buildTelegramWebAppUrl(string $path): string
    {
        ...
        // Создаем URL для Telegram WebApp
        $telegramUrl = "https://t.me/" . config('services.telegram.bot_username') . "?startapp=" . urlencode($path);
        
        Log::info('Построен Telegram WebApp URL', [
            'path' => $path,
