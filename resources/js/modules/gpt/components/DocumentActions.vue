...
const loadUserContactData = async () => {
    try {
        ...
        if (user) {
            // Проверяем, является ли email автогенерированным или отсутствует
            const isAutoGenerated = !user.email || user.email.endsWith('@auto.user');
...
const handleSubscriptionPayment = async () => {
    try {
        ...
        // console.log('=== Начало процесса оплаты абонемента ===');  // Закомментировано для продакшена
        ...
        // console.error('=== Ошибка при оплате абонемента ===');  // Закомментировано для продакшена
